...
const EditUsers = () => {
  ...
  useEffect(() => {
    ...
    .then((Response) => {
      ...
      setBanglaName(Response.data.BanglaName);
      ...
      setHSCResult(Response.data.HSCResult);
      setHSCCollegeName(Response.data.HSCCollegeName);
      setHSCRoll(Response.data.HSCRoll);
      setHSCBoard(Response.data.HSCBoard);
      setSSCResult(Response.data.SSCResult);
      setSSCCollegeName(Response.data.SSCCollegeName);
      setSSCRoll(Response.data.SSCRoll);
      setSSCBoard(Response.data.SSCBoard);
  ...
  const handleEditUser = () => {
    const data = {
      ...
      setLoading(true);
      axios.put(HTTPURL+'/users/'+id, data, { headers: { 'Content-Type': 'application/json' } })
      .then(() => {
        setLoading(false);
        navigate('/');
      })
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-4 py-4 bg-white shadow-md rounded-lg">
        <BackButton />
        <h1 className='text-3xl mx-auto'>Update User</h1>
      </div>
      {loading ? <Spinner /> : ''}
      <div className='py-4'>  
        <div className="">
